...
import sdp.ch11.cake.modell.Person
...
class UserComponentTest
  ...
  when(dao.getPeopleInClass(emptyClassName)).thenReturn(Nil)
  ...
  it should "properly handle an empty result." in {
    userService.getAverageAgeOfUsersInClass(emptyClassName) should equal(0.0d)
  }
}
